...
<ul>
    ...
    <li>
        ...
        <ul>
            <li>
            <b>Use a hashtable to track the the value (key) and index (value) of numbers we have visited.</b>\
            <i>Why a hashtable?</i>\
            The hashtable can be used to lookup data in constant time.\
            \
            <i>Why do we need to store the number and its index?</i>\
            For each iteration, we will know the complement number of the pair we are looking for. We can use the hashtable to
            check, in constant time O(1), if we have already seen that number.
            </li>
            <li>
            ...
            <li>
                ...
                <blockquote>
                🧠 I need to find an index in the array without looping through again. I know I can get a constant time lookup using a hashtable. What can I compute as my key that will give me the index I need?
                </blockquote>
            </li>
        </ul>
    </li>
